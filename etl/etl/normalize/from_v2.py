...
def normalize_v2(raw: dict[str, DataFrame], prefix: str = "v2") -> dict[str, DataFrame]:
    ...
    if "core.users" in raw:
        ...
        usuarios = (
            ...
            .select(
                "id",
                F.col("telegram_id") if "telegram_id" in df.columns else F.lit(None).cast("string").alias("telegram_id"),
                F.col("username") if "username" in df.columns else F.lit(None).cast("string").alias("username"),
    ...
    if "core.events" in raw:
        ...
        eventos = (
            ...
            .withColumn("vip_pull", F.col("vip_pull").cast(DoubleType()) if "vip_pull" in df.columns else F.lit(None).cast(DoubleType()))
            .withColumn("event_date", F.col("event_date").cast(DateType()) if "event_date" in df.columns else F.lit(None).cast(DateType()))
            .withColumn("start_time", F.col("start_time").cast(TimestampType()) if "start_time" in df.columns else F.lit(None).cast(TimestampType()))
            .withColumn("end_time", F.col("end_time").cast(TimestampType()) if "end_time" in df.columns else F.lit(None).cast(TimestampType()))
            .withColumn("event_state_id", F.col("event_state_id").cast(IntegerType()) if "event_state_id" in df.columns else F.lit(None).cast(IntegerType()))
            .withColumn("name", F.col("name") if "name" in df.columns else F.col("event_name").alias("name") if "event_name" in df.columns else F.lit(None).cast("string"))
            .withColumn("event_type", F.col("event_type") if "event_type" in df.columns else F.lit(None).cast("string"))
            .withColumn("expected_demand_level", F.col("expected_demand_level") if "expected_demand_level" in df.columns else F.lit(None).cast("string"))
            .withColumn("source", F.lit(prefix))
